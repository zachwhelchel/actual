...
export function CoachProvider({ budgetId, children }) {
  ...
  const dialogueId_key = 'dialogueId_' + budgetId.budgetId;

  const nickSmith_myBudgetCoachReason_key =
    'nickSmith_myBudgetCoachReason_' + budgetId.budgetId;
  const nickSmith_accountStructure_key =
    'nickSmith_accountStructure_' + budgetId.budgetId;
  const nickSmith_budgetWithPartner_key =
    'nickSmith_budgetWithPartner_' + budgetId.budgetId;
  const nickSmith_moreCardsCount_key =
    'nickSmith_moreCardsCount_' + budgetId.budgetId;
  const nickSmith_currentBalanceCalculation_key =
    'nickSmith_currentBalanceCalculation_' + budgetId.budgetId;
  const nickSmith_runningBalance_key =
    'nickSmith_runningBalance_' + budgetId.budgetId;
  //a lot of these need to be per account as well.

  const [dialogueId, setDialogueId] = useState(
    localStorage.getItem(dialogueId_key) ?? '',
  );

  const [nickSmith_myBudgetCoachReason, setNickSmith_myBudgetCoachReason] =
    useState(localStorage.getItem(nickSmith_myBudgetCoachReason_key) ?? '');
  const [nickSmith_accountStructure, setNickSmith_accountStructure] = useState(
    localStorage.getItem(nickSmith_accountStructure_key) ?? '',
  );
  const [nickSmith_budgetWithPartner, setNickSmith_budgetWithPartner] =
    useState(localStorage.getItem(nickSmith_budgetWithPartner_key) ?? '');
  const [nickSmith_moreCardsCount, setNickSmith_moreCardsCount] = useState(
    localStorage.getItem(nickSmith_moreCardsCount_key) ?? '',
  );
  const [
    nickSmith_currentBalanceCalculation,
    setNickSmith_currentBalanceCalculation,
  ] = useState(
    localStorage.getItem(nickSmith_currentBalanceCalculation_key) ?? '',
  );
  const [nickSmith_runningBalance, setNickSmith_runningBalance] = useState(
    localStorage.getItem(nickSmith_runningBalance_key) ?? '',
  );
  ...
  useEffect(() => {
    localStorage.setItem(
      nickSmith_myBudgetCoachReason_key,
  ...
  useEffect(() => {
    localStorage.setItem(
      nickSmith_accountStructure_key,
  ...
  useEffect(() => {
    localStorage.setItem(
      nickSmith_budgetWithPartner_key,
  ...
  useEffect(() => {
    localStorage.setItem(
      nickSmith_currentBalanceCalculation_key,
      nickSmith_currentBalanceCalculation,
    );
  }, [nickSmith_currentBalanceCalculation]);
...
export default function Coach({
  ...
  const setupAsRenter = async () => {
    const group = categoryGroups.find(g => g.name === 'Bills');
    ...
    const id = await onSaveCategory(category, true);
    await moveTo(id);
  ...
  const setupAsOwner = async () => {
    ...
    const id = await onSaveCategory(category);
    await moveTo(id);
  ...
  const nickSmithACT1 = async () => {
    ...
    setOffset(100);
    if (
      nickSmith_accountStructure === '1and1' ||
      nickSmith_accountStructure === 'moreThanOneCC'
    ) {
  ...
  const addTaxesAndInsuranceAsOwner = async () => {
    ...
    const ids = await onSaveNewCategories([category, category2], true);
  ...
  const addUtilities = async () => {
    ...
    if (typeALevel === 0) {
      ...
      const ids = await onSaveNewCategories(
  ...
  if (dialogueId === 'nicktrue_1') {
    ...
    content = (
      <div>
        Welcome to MyBudgetCoach! MyBudgetCoach is a unique budgeting software
        that comes along with a virtual avatar. You selected, me, Nick Smith, as
        your personal coach. I look forward to helping you start a new phase of
    ...
    content = (
      <div>
        Before we begin the set-up process, I just want to let you know about
        some aspects of MyBudgetCoach.
        <Button
          type="primary"
          style={{ marginTop: 8 }}
    ...
    content = (
      <div>
        If at any time you reach a point where you would like a one-on-one
        coaching session with me, please select the link to the left and select
        a time that works for you. Please note, one-on-one coaching does have an
        additional hourly fee, in addition to your MyBudgetCoach subscription.
        <Button type="primary" style={{ marginTop: 8 }} onClick={() => nick3()}>
          Got It
        </Button>
      </div>
    );
    ...
    content = (
      <div>
        As your virtual coach, I will be available whenever you are. You can
        complete set-up steps all at once, or you can complete them over time.
        Please move at your pace, and whenever you are ready for the next step
        of our work together, I will be here to help you out.
        <Button
          type="primary"
          style={{ marginTop: 8 }}
    ...
    content = (
      <div>
        To personalize your experience setting up MyBudgetCoach, I will ask you
        questions along the way. These questions will shift my coaching approach
        so I am helping you the best way for your specific goals.
        <Button
          type="primary"
          style={{ marginTop: 8 }}
    ...
    content = (
      <div>
        ...
        <p>
          We will start off with the following steps for the Account Set-Up
          stage.
        </p>
        <ol>
    ...
    content = (
      <div>
        Before we start adding accounts, please answer two questions so I can
        tailor my guidance to your financial infrastructure.
        <Button
          type="primary"
          style={{ marginTop: 8 }}
    ...
    content = (
      <div>
        Which account structure accurately represents your current financial
        set-up?
        <Button
          type="primary"
          style={{ marginTop: 8 }}
        ...
        >
          I use my credit card for day-to-day spending and my bank account for
          routine bills.
        </Button>
        <Button
          type="primary"
    ...
    content = (
      <div>
        ...
        >
          Yes and we have consolidated finances and expenses are paid out of
          joint accounts.
        </Button>
        <Button
          type="primary"
    ...
    if (
      nickSmith_accountStructure === 'noCC' &&
      (nickSmith_budgetWithPartner === 'justMe' ||
        nickSmith_budgetWithPartner === 'jointSeparate')
    ) {
      content = (
        <div>
      ...
      nickSmith_accountStructure === 'noCC' &&
      nickSmith_budgetWithPartner === 'jointConsolidated'
    ) {
      content = (
        <div>
          Based on your answers, I recommend adding just your joint checking
          account.
          <p>{`Since you don't have any other accounts where expenses are paid out of, one account is enough!`}</p>
          <Button
            type="primary"
      ...
      content = (
        <div>
          Based on your answers, I recommend adding your checking account first
          and then your main credit card account.
          <Button
            type="primary"
            style={{ marginTop: 8 }}
            onClick={() => setDialogueId('nicksmith_15')}
      ...
      content = (
        <div>
          Based on your answers, I recommend adding your joint checking account
          first and then your main joint credit card account.
          <Button
            type="primary"
            style={{ marginTop: 8 }}
            onClick={() => setDialogueId('nicksmith_15')}
    ...
    if (nickSmith_budgetWithPartner === 'jointConsolidated') {
      content = (
        <div>
          Please login to your bank now so you can gather your current balance
    ...
    if (nickSmith_budgetWithPartner === 'jointConsolidated') {
      content = (
        <div>
          Please select Create Local Account. Name your joint account, and then
          enter the current balance of your joint checking account.
          <Button
            type="primary"
            style={{ marginTop: 8 }}
      ...
      content = (
        <div>
          Please select the Create Local Account option. Name your account, and
          then enter the current balance of your checking account.
          <Button
            type="primary"
            style={{ marginTop: 8 }}
            onClick={() => nickSmithCCCheck()}
    ...
    if (nickSmith_accountStructure === 'noCC') {
      content = (
        <div>
          Great job adding your account! You may be thinking: Let me add in my
      ...
      content = (
        <div>
          Great job adding your accounts! You may be thinking: Let me add in my
          savings account next.
          <Button
            type="primary"
            style={{ marginTop: 8 }}
    ...
    content = (
      <div>
        Understood; For now, we are going to hold off adding account(s) that are
        owned and managed only by you. The budget we will be establishing is for
    ...
    content = (
      <div>
        If at a later point you would like to add your own accounts, please
        schedule a one-on-one session with me and I would be happy to help you
        add the account and discuss strategies for organizing the budget.
        <Button
          type="primary"
          style={{ marginTop: 8 }}
    ...
    content = (
      <div>
        Although you may have a joint savings account, as you first start using
        MyBudgetCoach, I recommend to not link the Savings account at this
        stage.
        <Button
          type="primary"
          style={{ marginTop: 8 }}
    ...
    content = (
      <div>
        Although you may have a savings account, as you first start using
        MyBudgetCoach, I recommend to not link the Savings account at this
        stage.
        <Button
          type="primary"
    ...
    content = (
      <div>
        If you want to further understand how to easily use saving accounts in
        MyBudgetCoach, please schedule a one-on-one coaching session with me.
        <Button
          type="primary"
          style={{ marginTop: 8 }}
    ...
    content = (
      <div>
        You are all done adding accounts. Do you feel the momentum building
    ...
    if (nickSmith_budgetWithPartner === 'jointConsolidated') {
      content = (
        <div>
          ...
          <p>
            Please login to your bank so you can gather your current credit card
            balance.
          </p>
          <Button
            type="primary"
    ...
    content = (
      <div>
        Please select + Add account and then Create Local Account. Name your
        account, and then enter the current balance of your credit card.
    ...
    content = (
      <div>
        You selected earlier that you have more than one credit card. Let me ask
        some questions to understand more about your current financial
        structure.
        <Button
          type="primary"
          style={{ marginTop: 8 }}
    ...
    if (nickSmith_budgetWithPartner === 'jointConsolidated') {
      content = (
        <div>
          Congratulations on adding three joint accounts! As you continue to use
          MyBudgetCoach, please keep in mind each account requires
          reconciliation, a few times a week, and categorizing of transactions.
          <Button
            type="primary"
            style={{ marginTop: 8 }}
      ...
      content = (
        <div>
          Congratulations on adding three accounts! As you continue to use
          MyBudgetCoach, please keep in mind each account requires
          reconciliation, a few times a week, and categorizing of transactions.
          <Button
            type="primary"
    ...
    content = (
      <div>
        How many transactions per week occur on your remaining credit card
        account?
        <Button
          type="primary"
          style={{ marginTop: 8 }}
    ...
    content = (
      <div>
        Based on the number of transactions that occur per week, I would
        recommend leaving out the additional account for now, and then if you
        would like to add it on a future date, feel free!
        <Button
          type="primary"
          style={{ marginTop: 8 }}
    ...
    content = (
      <div>
        How many transactions per week occur on your next most highly used
        credit card of your remaining accounts?
        <Button
          type="primary"
          style={{ marginTop: 8 }}
    ...
    if (nickSmith_budgetWithPartner === 'jointConsolidated') {
      content = (
        <div>
          Based on the number of transactions that occur per week, I would
          recommend not adding the remainder of your joint credit card accounts
          for now.
          <Button
            type="primary"
            style={{ marginTop: 8 }}
      ...
      content = (
        <div>
          Based on the number of transactions that occur per week, I would
          recommend leaving out the remainder of your credit card accounts for
          now, and then if you would like to add additional ones on a future
          date, feel free!
          <Button
            type="primary"
            style={{ marginTop: 8 }}
    ...
    content = (
      <div>
        To summarize, you have now added a total of four accounts. Prior to
        linking a fifth account, I am going to ask you the same question as
        before, for the frequency of transactions.
        <Button
          type="primary"
          style={{ marginTop: 8 }}
    ...
    content = (
      <div>
        Please note that linking many accounts leads to a complicated use of
        MyBudgetCoach and could lead to feeling overwhelmed each time you log
        in.
        <Button
          type="primary"
          style={{ marginTop: 8 }}
    ...
    if (nickSmith_budgetWithPartner === 'jointConsolidated') {
      content = (
        <div>
          Based on the number of transactions that occur per week, I would
          recommend leaving out the additional joint account.
          <Button
            type="primary"
            style={{ marginTop: 8 }}
            onClick={() => setDialogueId('nicksmith_wrap_accounts_jc')}
    ...
    content = (
      <div>
        Alright! That completes the overview of the account screen. Are you
        starting to feel like a MyBudgetCoach expert!?
        <Button
          type="primary"
          style={{ marginTop: 8 }}
    ...
    content = (
      <div>
        To verify your understanding of the account screen, I will now walk you
        through adding two test transactions to the checking account.
        <Button
          type="primary"
          style={{ marginTop: 8 }}
    ...
    content = (
      <div>
        Notice that when you add a new transaction, the system defaults the
        checkmark symbol to a non-filled color which means the transaction is
        not yet cleared.
        <Button
          type="primary"
          style={{ marginTop: 8 }}
    ...
    content = (
      <div>
        Now that you have added two transactions, please take note that the
        payment to the grocery store has been cleared, but the income has not
        been cleared.
        <Button
          type="primary"
          style={{ marginTop: 8 }}
    ...
    content = (
      <div>
        Please select the Account Balance to view the Cleared and Uncleared
        total.
        <Button
          type="primary"
          style={{ marginTop: 8 }}
    ...
    content = (
      <div>
        ...
        <p>
          Please select the grey check mark to make it green to clear the
          transaction.
        </p>
        <Button
          type="primary"
    ...
    content = (
      <div>
        The Cleared total now matches the account balance and the Uncleared
        total was updated to 0.
        <Button
          type="primary"
          style={{ marginTop: 8 }}
    ...
    content = (
      <div>
        In this case, you could add the transaction but leave the checkmark
        grey, uncleared, until a later date when the transaction has cleared at
        your bank.
        <Button
          type="primary"
          style={{ marginTop: 8 }}
    ...
    content = (
      <div>
        The important difference between the two types of accounts is that in a
        credit card account, Payments increase the balance, and Deposits
        decrease the balance. This is because a credit card balance is negative.
        <Button
          type="primary"
          style={{ marginTop: 8 }}
    ...
    content = (
      <div>
        Please select your main credit card account to add two hypothetical
        transactions.
        <Button
          type="primary"
          style={{ marginTop: 8 }}
    ...
    content = (
      <div>
        The credit card balance, a red negative number, has now increased by
        100.
        <Button
          type="primary"
          style={{ marginTop: 8 }}
    ...
    content = (
      <div>
        Please take note that the Payment field is populated for the checking
        account, which reduces the balance. However, the credit card account has
        the Deposit field populated because an inflow to a negative balance
    ...
    content = (
      <div>
        The last step of the Account overview phase is to confirm the amount of
        the starting balance for each added account.
        <Button
          type="primary"
          style={{ marginTop: 8 }}
    ...
    content = (
      <div>
        When re-creating a copy of your financial institution transactions and
        balance in MyBudgetCoach, one of the most important pieces is knowing
        how the amount you see when you login to your bank account is
        calculated. Let me explain.
        <Button
          type="primary"
          style={{ marginTop: 8 }}
    ...
    content = (
      <div>
        Some financial institutions calculate the balance shown in a bank
        account after taking into account pending transactions. Others calculate
        the current balance by only including transactions that have cleared.
        <Button
          type="primary"
          style={{ marginTop: 8 }}
    ...
    content = (
      <div>
        Ok great! For this portion of the account set-up, I will be asking you
        questions along the way. Your responses will be used to tailor my
        guidance.
        <Button
          type="primary"
          style={{ marginTop: 8 }}
    ...
    content = (
      <div>
        Ok; This means that the account balance in MyBudgetCoach may not match
    ...
    content = (
      <div>
        Does your current balance match the amount in your MyBudgetCoach
        account, which is the amount entered for the Starting Balance?
        <Button
          type="primary"
          style={{ marginTop: 8 }}
    ...
    content = (
      <div>
        Please first total up your pending transactions. For outflows, please
        total as a negative number.
        <Button
          type="primary"
          style={{ marginTop: 8 }}
    ...
    content = (
      <div>
        This means if your pending transactions are negative, you would be
        adding them to increase your balance. If your pending transactions were
        positive, it would reduce your balance.
        <Button
          type="primary"
          style={{ marginTop: 8 }}
    ...
    content = (
      <div>
        Please enter this new amount into the Starting Balance transaction for
    ...
    content = (
      <div>
        Now, when you import those pending transactions at a later date, your
        starting balance will remain in-synch with your bank, versus double
        counting the pending transactions.
        <Button
          type="primary"
          style={{ marginTop: 8 }}
    ...
    content = (
      <div>
        Please remember, you will need to complete this small math exercise each
        time you reconcile this account if there are any pending transactions.
        <Button
          type="primary"
          style={{ marginTop: 8 }}
          onClick={() => nickSmithOptionNE1()}
    ...
    content = (
      <div>
        Please adjust your starting balance amount to the balance you see in
        your bank account.
        <Button
          type="primary"
          style={{ marginTop: 8 }}
    ...
    content = (
      <div>
        Great work! Correcting your Starting Balance now will save much time
        later on during future reconciliation processes.
        <Button
          type="primary"
          style={{ marginTop: 8 }}
          onClick={() => nickSmithOptionNE1()}
    ...
    content = (
      <div>
        Ok; This means that the account balance in MyBudgetCoach should match
        your financial institution. This assumes all transactions cleared in the
        bank account are entered in MyBudgetCoach.
        <Button
          type="primary"
          style={{ marginTop: 8 }}
    ...
    content = (
      <div>
        Does your current balance match the amount in your MyBudgetCoach account
        that you entered for the Starting Balance?
        <Button
          type="primary"
          style={{ marginTop: 8 }}
          onClick={() => setDialogueId('nicksmith_vab_11')}
    ...
    content = (
      <div>
        I completely understand. This is a question you may have not considered
        before. Let me ask some questions to learn more about your specific
        bank.
        <Button
          type="primary"
          style={{ marginTop: 8 }}
    ...
    content = (
      <div>
        Please locate the running balance for the last cleared transaction.
        Please exclude any pending transactions.
        <Button
          type="primary"
          style={{ marginTop: 8 }}
    ...
    content = (
      <div>
        Thanks for this answer. This means that your bank account balance does
        not take into account pending transactions.
        <Button
          type="primary"
          style={{ marginTop: 8 }}
          onClick={() => setDialogueId('nicksmith_vab_26')}
    ...
    content = (
      <div>
        Thanks for this answer. This means that your bank account balance does
        take into account pending transactions.
        <Button
          type="primary"
          style={{ marginTop: 8 }}
    ...
    content = (
      <div>
        Understood; This means we need to update your Starting Balance. The
        MyBudgetCoach Starting Balance should be equal to the account balance
        shown after the most recent cleared transaction.
        <Button
          type="primary"
          style={{ marginTop: 8 }}
    ...
    content = (
      <div>
        Please adjust your starting balance amount to the running balance amount
        shown as of the most recent cleared transaction.
        <Button
          type="primary"
          style={{ marginTop: 8 }}
    ...
    content = (
      <div>
        Does the balance shown after the most recent cleared transaction match
        the amount you entered for the Starting Balance in MyBudgetCoach?
        <Button
          type="primary"
          style={{ marginTop: 8 }}
